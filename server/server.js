...
app.notify('/notify_build_result', async (req, res) => {
    ...
    try {
        await filehandle.mkdir(
            `./server/data/builds/build-${buildId}-${status}`,
            { recursive: true }
        );
        await filehandle.writeFile(
            `./server/data/builds/build-${buildId}-${status}/info.json`,
            JSON.stringify({
                buildStart,
                buildFinish,
                commitHash,
...
app.post('/start_build', async (req, res) => {
    ...
    startBuildOnAgent(commitHash, command)
        .then(resolve => {
            res.sendStatus(201);
        })
        .catch(err => {
...
app.get('/builds', async (req, res) => {
    ...
    for (const build of builds) {
        let buildInfo = await filehandle.readFile(
            `${buildsPath}/${build}/info.json`
        );
